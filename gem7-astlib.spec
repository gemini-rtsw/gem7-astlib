...
# IOC does `ld < .../astlib/V1-4/bin/ppc604/astlib` at boot, so the copy on
...
# "gem7-astlib" could not satisfy that -- dnf would delete V1-9-4 to install
...
%global supdir  /gemini/epics3.13.4/support/astlib
...
%global expected_name gem7-astlib-%{libdir}
...
Summary:        astlib %{version} for GEM7, cross-compiled for vxWorks ppc604
...
# astlib compiles against both slalib and timelib. Pinned exactly: the
# versions built against are the versions whose objects the crate ld's.
BuildRequires:  gem7-slalib-V1-9-4-devel = 1.9.4-1.git008125e%{?dist}
BuildRequires:  gem7-timelib-V1-8-6-devel = 1.8.6-1.git4790eea%{?dist}
...
%package devel
Summary:        Headers for building against astlib %{version}
Provides:       gem7-astlib-devel = %{version}
%description devel
...
%changelog
* Mon Sep 14 2026 Hawi Stecher <hawi.stecher@noirlab.edu> - 1.4-1
- First packaged build. Source is CVS tag V1-4 from
